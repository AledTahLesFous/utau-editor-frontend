...
export class ProjectEditComponent implements OnInit {
  ...
  tempo = '';
  key_signature = '';
  duration: number = 0;      // valeur réelle du projet
  durationEdit: number = 0;  // valeur temporaire dans l’input
    ...
    this.http.get(`http://127.0.0.1:8055/items/projects?filter[title][_eq]=${projectName}`, {
      ...
      next: (res: any) => {
        if (res.data && res.data.length > 0) {
          ...
          this.duration = project.duration || 100; // valeur réelle
          this.durationEdit = this.duration;      // valeur temporaire pour l'input
